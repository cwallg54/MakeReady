...
import { createDesignForArtAction } from "@/lib/designs/actions";
...
/**
 * Inline "punch in the design" form for an art job. Composes the item number,
 * auto-assigns a barcode, attaches the art, and (via the action) creates the
 * orderable inventory item + links it to this art request.
 */
export function ArtDesignForm({
  brands, suffixes, requestId, orderId, defaultCustNumber, defaultBpId, defaultDescription,
}: {
  ...
  const [custNumber, setCustNumber] = useState(defaultCustNumber);
  ...
  return (
    <form action={createDesignForArtAction} className="space-y-4" encType="multipart/form-data">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <label>
          ...
          <input name="description" defaultValue={defaultDescription} placeholder="Summer Bloom LC Tan" className={inp} />
        </label>
        <label>
      ...
      <label className="block">
        <span className={lbl}>Art image (attaches to the item and this order)</span>
      ...
      <div className="rounded-md bg-neutral-50 px-3 py-2 text-xs text-neutral-500">
        {effectiveItem
          ? "Creates the inventory item with the art attached — sales can order it immediately — and links it to this art job so it can be approved."
          : "Without an item number this saves as a draft — the art job can't be approved until the item number and barcode are set (the gate)."}
      </div>

      <button className="rounded-md bg-neutral-900 px-5 py-2.5 text-sm font-semibold text-white hover:bg-neutral-700">Punch in design &amp; create item</button>
    </form>
  );
}
