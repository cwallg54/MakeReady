        ...
        <ul className="divide-y divide-neutral-100">
          ...
          {rows.map((c) => (
            <li key={c.id} className="flex items-center justify-between px-5 py-3">
              <div>
                <Link href={`/admin/automations/${c.id}`} className="font-medium text-neutral-900 hover:underline">{c.name}</Link>
              ...
              <Link href={`/admin/automations/${c.id}`} className="text-sm text-neutral-600 hover:text-neutral-900">Edit</Link>
            </li>
          ))}
        </ul>
      </Card>
