...
const PUBLIC_PATHS = ["/login", "/forgot-password", "/reset-password", "/lead", "/track", "/apply", "/schedule", "/proof", "/quote", "/shop"];
...
export async function proxy(req: NextRequest) {
  ...
  // stamps the nonce onto its scripts; also set CSP on the response. (The static
  // security headers — HSTS, nosniff, frame, permissions — live in next.config.)
...
export const config = {
  ...
  matcher: ["/((?!_next/static|_next/image|favicon.ico|manifest.webmanifest|sw.js|offline.html|api|.*\\.(?:svg|png|jpg|jpeg|gif|webp|ico|webmanifest)$).*)"],
};
