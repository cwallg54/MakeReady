...
_Updated September 14, 2026. Screenshots captured from the live application._
...
MakeReady is where the whole business now runs: the enquiry, the quote, the order, the art, the press, the invoice, the money in, the money out, and the books. This guide walks that whole path in the order the work actually happens, and for every step it says which screen you do it on.

Read Part 1 for the shape of the job on one page. Part 2 is the step-by-step, with a picture of each screen as it looks today. Part 3 is the daily, weekly and monthly rhythm. Part 4 covers admin and permissions.

> Every screenshot in this guide is generated from the live application. When a screen changes, the pictures are re-captured rather than redrawn, so what you see here is what you will see when you sign in.
...
- **The left-hand menu** is the whole platform. One section opens at a time — opening Accounting closes CRM — so the list stays short enough to read.
- **The search box at the top** searches everything at once: customers, contacts, invoices, bills, vendors, stock items and designs. Type a company name, an invoice number or a SKU.
...
# Part 1 — The job on one page

This is the whole path from a phone call to money in the bank and a closed month. Each row is a stage; the right-hand column is where you do it.
...
> Every order carries an **Order journey** strip showing exactly which of these stages it has reached. If you only ever learn one thing about an order, learn to read that strip.
...
1. CRM → **New Business Partner**.
1. Fill in the account, the primary contact, the address and the terms.
1. Save. They start as a **Lead**; promote them to Prospect and then Customer as the relationship moves.
...
- **Log a call, an email or a visit** with the buttons at the top — that is what feeds the rep activity report, so it is worth doing.
- **AI summary** reads the account's history and tells you where things stand before you ring them.
- **Draft email** writes a first draft you then edit; it is never sent without you sending it.
- **Invite to portal** gives the customer their own login to see orders, proofs and invoices.
- **New Quote** starts a quote with this customer already selected.
...
### Customers who are due to come back
...
1. **Add a line** and pick the item.
1. Type the **quantity** — the unit price fills in from the quantity band.
1. Pick the **size** for apparel; the upcharge is added automatically.
1. Tick the **charges and setup** that apply — screen prep, mid-run colour changes, art time, rush.
1. Set **Reorder** on a repeat job and the new-only screen prep drops off.
...
> Unit prices on band-priced items are deliberately locked — the same reason the grey cells on the old spreadsheets said 'Formulas – Do Not Type'. For a genuine one-off, set the line to **custom** and type your own price.
...
- **Order journey** across the top shows how far the job has got.
- **Update stage** moves it on: Order Received → Art & Proof → In Production → Quality Check → Shipped → Delivered.
- **Submit to art** raises the art request; **Send to production** puts it on the floor.
- **Items & decoration** is the specification the artist and the press both work from.
- **Customer tracker link** is a public link you can send so they can follow the job themselves without ringing you.
- **Create invoice** raises the invoice from the order when it ships.
...
1. **Artwork produced.**
1. **Out for review.**
1. **Customer approved** — the proof goes to the customer from this screen, and every proof sent is kept.
1. **Production ready** — this one only unlocks once the readiness checklist is complete.
- **Draft brief with AI** writes the customisation brief from the order specification; the artist edits it.
- **Production files & handoff** is where the press-ready files go.
- **Revisions** keeps every version, so 'which one did they approve?' always has an answer.
...
1. Inventory → **Purchase Orders** → **New PO**, pick the vendor and the lines.
1. Receive the goods against the PO when they arrive.
...
- **PDF** produces the document; **Email invoice** sends it to the customer.
- The emailed invoice carries a **pay link**: the customer can pay by ACH with no fee, or by card with the surcharge shown, without ringing anyone.
- **Record payment** takes a receipt straight against this invoice.
- **Void** reverses an invoice raised in error. Invoices are never deleted — the number stays, marked void, so the sequence has no holes.
...
1. **Record the receipt.** Against one invoice, across several, or on account if you do not yet know what it pays.
...
- **Call list — 15 days past due** is who to ring today.
- **Cards to run today** and **Cards past due** are the card-on-file accounts.
- **Prepay / COD outstanding** is work that should not have shipped without money.
- **Open AR by parent account** rolls the chains up, so a group with forty stores is one conversation.
...
- **Journal Entries** — every posting, whether it came from an invoice, a payment run, payroll or somebody's manual adjustment. Accruals reverse themselves automatically the following month.
- **Trial Balance** — every account's balance, debits equal to credits.
...
## 12. See how the business is doing
...
The built-in set covers the reports the business has always run:

- **Sales Analysis by Salesperson & Customer** — three years side by side, month by month.
- **Open Orders by Salesperson** and **by Type**.
- **Customer Credit Report** — the credit picture for an account.
...
1. Reports → **Build a report**.
1. Pick the source and the columns.
1. **Add filter** for each condition you want.
...
A saved report can be edited, deleted, exported to CSV and scheduled to email itself out.
...
- **Add from inventory** publishes an existing stocked item to the store.
- **Categories**, **Promos** and **Groups** control how it is presented and who sees what price.
...
# Part 3 — The rhythm of the job


## Every day

- **Dashboard** — what is open, what is on the floor, what is low on stock.
- **Collections → call list** — ring today's 15-days-past-due accounts; run the cards that are due.
- **Payments** — record what came in.
- **Art queue** and **Production schedule** — anything stuck, anything due to ship.
- **Approvals inbox** — decisions waiting on you.


## Every week

- **Weekly flash** — the week's cash, sales, receipts and anything waiting on someone.
- **Deposits** — bank the receipts; nothing should sit undeposited over a weekend.
- **Payment run** — pay the bills that are due, netted against vendor credits.
- **Reorder forecast** — order the blanks before the days-left column goes red.
- **GRNI** — chase vendor bills for goods already received.


## Twice a month

- **Payroll** — start the run from the template, check it, post it.


## Every month

1. **Bank reconciliation** — import the statement and clear it.
1. **Sales tax filing** — run the period and file it.
1. **GRNI and accruals** — make sure costs sit in the month that earned them.
1. **Trial balance** — check it balances and nothing looks wrong.
1. **Income statement, balance sheet, cash flow** — run and save them.
1. **Period close** — work the close screen, then lock the month.


## Every quarter

- **Quarterly pack** — the quarter against last year, by segment.
- **Sales goals** and **Commission** — how the reps did, and what they earned.
- **Profitability and job costing** — which customers and which work are actually worth having.


---


# Part 4 — Running the place
...
Rights do not cascade upward: a view grant never confers edit, and edit never confers delete.
...
# Appendix — What came across from SAP

MakeReady was not started empty. The SAP history was migrated, so the reports have real history behind them from the first day:
...
Invoices carried over keep their SAP number with an **SI-** prefix, credit memos **SC-**, and vendor bills **SAP-**, so a customer or supplier quoting an old document number can still be found. Their balances were carried without re-posting to the ledger, because the ledger already holds that history — which is why the accounts tie exactly rather than double-counting.


## Getting help

The **Help** section in the left-hand menu holds the how-to articles. Anything that looks wrong in the data — rather than in the way a screen works — is worth raising with an administrator, who can check the audit log to see exactly what happened and when.
